...
class MultiDiscoveryManager:
    ...
    async def _try_network_scan(self, timeout: int = 5) -> Optional[DiscoveredNode]:
        ...
        try:
            ...
            logger.info(f"🔍 네트워크 대역 스캔: {network_prefix}.0/24")
            ...
            for i in range(1, 255):
                ...
                if ip == local_ip:
                    continue
                tasks.append(self._check_airclass_node(ip, 8000))

            # 타임아웃 내에 첫 번째로 발견된 메인 노드 반환
            try:
    ...
    async def _try_common_ips(self) -> Optional[DiscoveredNode]:
        ...
        for ip in common_ips:
            ...
            node = await self._check_airclass_node(
                ip if ip != "localhost" else "127.0.0.1"
            )
            if node:
                return node
