...
class EngagementCalculator:
    ...
    def detect_confusion(
        ...
        # 1. Low accuracy + high chat activity
        if quiz_accuracy < 0.5 and chat_activity_high:
            confidence += 0.5

        # 2. Explicit confusion indicators
        if confusion_indicators:
            confidence += 0.3 + (len(confusion_indicators) * 0.1)
    ...
    @staticmethod
    def interpret_engagement_level(score: float) -> Dict[str, str]:
        """
        참여도 점수 해석
...
class EngagementTracker:
    ...
    async def track_activity(
        ...
        try:
            ...
            # 마지막 활동 시간 업데이트
            engagement.metrics.last_activity_time = datetime.utcnow()
