...
from services.vod_service import get_vod_storage
...
def get_storage():
    ...
    if not storage:
        raise HTTPException(status_code=503, detail="VOD storage not initialized")
    return storage
...
async def get_video_info(video_id: str, storage=Depends(get_storage)):
    """
    비디오 정보 조회
...
async def get_thumbnail(video_id: str, storage=Depends(get_storage)):
    """
    비디오 썸네일
...
async def delete_video(video_id: str, storage=Depends(get_storage)):
    """
    비디오 삭제
...
async def get_chapters(video_id: str, storage=Depends(get_storage)):
    """
    챕터 목록
