...
from services.recording_service import get_recording_manager
...
async def start_recording(
    ...
    try:
        result = manager.start_recording(
            session_id=session_id, stream_url=stream_url, output_format=output_format
        )
...
@router.post("/stop")
async def stop_recording(recording_id: str, manager=Depends(get_manager)):
    """
    스트림 녹화 중지
...
async def get_recording_status(recording_id: str, manager=Depends(get_manager)):
    """
    녹화 상태 조회
...
async def list_session_recordings(session_id: str, manager=Depends(get_manager)):
    """
    세션별 녹화 목록
...
@router.get("")
async def list_all_recordings(manager=Depends(get_manager)):
    """
    모든 녹화 조회
...
async def delete_recording(recording_id: str, manager=Depends(get_manager)):
    """
    녹화 파일 삭제
